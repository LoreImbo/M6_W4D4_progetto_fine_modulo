	...
	private void Shooting() {
		...
		if (Physics.Raycast (VFX.muzzle.position, VFX.muzzle.transform.forward, out hit, parameters.radius)) {
			if (CheckTags (hit.collider) == true) {
				Shot ();
				   STT_Actor actor = hit.collider.GetComponent<STT_Actor>();
				   if (actor != null) {
					   actor.ReceiveDamage(parameters.power, hit.point);
				   } else {
					   Debug.LogWarning($"STT_Actor component not found on {hit.collider.gameObject.name}");
				   }
			}
